                ...
                memInfo.Body.FinishCondition(
                        Ast.Throw(
                            Ast.New(
                                ...
                                AstUtils.Constant(string.Format("Setting field {0} on value type {1} may result in updating a copy.  Use {1}.{0}.SetValue(instance, value) if this is safe.  For more information help({1}.{0}.SetValue).", field.Name, field.DeclaringType.Name))
                            ),
                            typeof(object)
                        )
                    );
